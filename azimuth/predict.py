...
def print_summary(global_metric, results, learn_options, feature_sets, flags):
    ...
    print(("\t\tmedian %s across folds=%.2f" %
           (learn_options['metric'], np.median(results[0]))))
    print(("\t\torder=%d" % learn_options["order"]))
    if 'kerntype' in learn_options:
        "\t\tkern type = %s" % learn_options['kerntype']
    if 'degree' in learn_options:
        print(("\t\tdegree=%d" % learn_options['degree']))
...
def extract_spearman_for_fold(metrics, fold, i, predictions, truth, y_ground_truth, test, y_pred, learn_options):
    spearman = util.spearmanr_nonan(
        y_ground_truth[test].flatten(), y_pred.flatten())[0]
...
def get_train_test(test_gene, y_all, train_genes=None):
    ...
    not_test = (y_all.index.get_level_values(
        'Target gene').values != test_gene)
    ...
    if train_genes is not None:
        ...
        for t_gene in train_genes:
            in_train_genes = np.logical_or(
                in_train_genes, (y_all.index.get_level_values('Target gene').values == t_gene))
    ...
    if test_gene == 'dummy':
        ...
        test = (y_all.index.get_level_values(
            'Target gene').values == test_gene)
...
def cross_validate(y_all, feature_sets, learn_options=None, TEST=False, train_genes=None, CV=True):
    ...
    print(("range of y_all is [%f, %f]" % (np.min(y_all[learn_options['target_name']].values), np.max(
        y_all[learn_options['target_name']].values))))
    ...
    assert learn_options["method"] == "linreg" and learn_options['penalty'] == 'L2' or learn_options[
    ...
    y = np.array(y_all[learn_options["target_name"]
    ...
    if learn_options["cv"] == "stratified":
        assert "extra_pairs" not in learn_options or learn_options[
        ...
        if not CV:
            ...
            train_test_tmp = get_train_test('dummy', y_all)
            ...
            for i, gene in enumerate(learn_options['test_genes']):
                cv.append(get_train_test(
                    gene, y_all, learn_options['train_genes']))
            fold_labels = learn_options["test_genes"]
            # if train and test genes are seperate, there should be only one fold
            train_test_disjoint = set.isdisjoint(set(
                learn_options["train_genes"].tolist()), set(learn_options["test_genes"].tolist()))
        ...
        if learn_options['num_genes_remove_train'] is not None:
            for i, (train, test) in enumerate(cv):
                ...
                genes_to_keep = unique_genes[0:len(
                    unique_genes) - learn_options['num_genes_remove_train']]
                guides_to_keep = []
                filtered_train = []
                ...
                print(("# train/train after/before is %s, %s" %
                       (len(cv[i][0]), len(cv_i_orig[0]))))
                print(("# test/test after/before is %s, %s" %
                       (len(cv[i][1]), len(cv_i_orig[1]))))
    else:
    ...
    truth = dict([(t, dict([(m, np.array([]))
                            for m in ['raw', 'ranks', 'thrs']])) for t in fold_labels])
    ...
    if num_proc > 1:
        ...
        for i, fold in enumerate(cv):
            ...
            if learn_options["method"] == "GPy":
                job = pool.apply_async(azimuth.models.GP.gp_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "linreg":
                job = pool.apply_async(azimuth.models.regression.linreg_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "logregL1":
                job = pool.apply_async(azimuth.models.regression.logreg_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "AdaBoostRegressor":
                job = pool.apply_async(azimuth.models.ensembles.adaboost_on_fold, args=(
                ...
                job = pool.apply_async(azimuth.models.ensembles.decisiontree_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "RandomForestRegressor":
                job = pool.apply_async(azimuth.models.ensembles.randomforest_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "ARDRegression":
                job = pool.apply_async(azimuth.models.regression.ARDRegression_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "random":
                job = pool.apply_async(azimuth.models.baselines.random_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "mean":
                job = pool.apply_async(azimuth.models.baselines.mean_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "SVC":
                job = pool.apply_async(azimuth.models.baselines.SVC_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "DNN":
                job = pool.apply_async(azimuth.models.DNN.DNN_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "lasso_ensemble":
                job = pool.apply_async(azimuth.models.ensembles.LASSOs_ensemble_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "doench":
                job = pool.apply_async(azimuth.models.baselines.doench_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "sgrna_from_doench":
                job = pool.apply_async(azimuth.models.baselines.sgrna_from_doench_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            elif learn_options["method"] == "xu_et_al":
                job = pool.apply_async(azimuth.models.baselines.xu_et_al_on_fold, args=(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options))
            else:
        ...
        for i, fold in enumerate(cv):  # i in range(0,len(jobs)):
            ...
            if learn_options["training_metric"] == "AUC":
                extract_fpr_tpr_for_fold(
                    metrics, fold_labels[i], i, predictions, truth, y_all[learn_options["ground_truth_label"]].values, test, y_pred)
            elif learn_options["training_metric"] == "NDCG":
                extract_NDCG_for_fold(metrics, fold_labels[i], i, predictions, truth,
                                      y_all[learn_options["ground_truth_label"]].values, test, y_pred, learn_options)
            elif learn_options["training_metric"] == 'spearmanr':
                extract_spearman_for_fold(
                    metrics, fold_labels[i], i, predictions, truth, y_all[learn_options["ground_truth_label"]].values, test, y_pred, learn_options)
            else:
                raise Exception(
                    "invalid 'training_metric' in learn_options: %s" % learn_options["training_metric"])
        ...
        for i, fold in enumerate(cv):
            ...
            if learn_options["method"] == "GPy":
                y_pred, m[i] = gp_on_fold(
                    azimuth.models.GP.feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options)
            elif learn_options["method"] == "linreg":
                ...
                y_pred, m[i] = azimuth.models.ensembles.adaboost_on_fold(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options, classification=False)
            elif learn_options["method"] == "AdaBoostClassifier":
                y_pred, m[i] = azimuth.models.ensembles.adaboost_on_fold(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options, classification=True)
            elif learn_options["method"] == "DecisionTreeRegressor":
                ...
                y_pred, m[i] = azimuth.models.ensembles.LASSOs_ensemble_on_fold(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options)
            elif learn_options["method"] == "doench":
                y_pred, m[i] = azimuth.models.baselines.doench_on_fold(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options)
            elif learn_options["method"] == "sgrna_from_doench":
                y_pred, m[i] = azimuth.models.baselines.sgrna_from_doench_on_fold(
                    feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options)
            elif learn_options["method"] == "xu_et_al":
            ...
            print(("\t\tSpearman correlation: ",
                   util.spearmanr_nonan(y[test], y_pred)[0]))
            print(("\t\tfinished fold/gene %i of %i" %
                   (i + 1, len(fold_labels))))
